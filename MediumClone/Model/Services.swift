...
class ArticleServiceImp : ArticleService {
    func getFeedComments(slug: String) -> AnyPublisher<Comments, AFError> {
        return AF.request(buildUrl("comments", slug),method: .get,headers: [HTTPHeader(name: "Content-Type", value: "Application/json")]).cURLDescription(calling: {
            Logger.i($0)
    ...
    func getFeedDetail(slug: String) -> AnyPublisher<ArticleDetail,AFError>{
        return AF.request(buildUrl("articles", slug),method: .get,headers: [HTTPHeader(name: "Content-Type", value: "Application/json")]).cURLDescription(calling: {
            Logger.i($0)
